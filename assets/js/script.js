...
var displayPrevCityButtons = function () {
  ...
  for (var z = 0; z < 4; z++) {
    ...
    console.log(futureCities)
    prevCityEl.innerHTML = futureCities;
    prevCityEl.classList = "btn";
    var dataCity = "data-city" + [z];
    prevCityEl.setAttribute("data", [dataCity]);
    PrevCityBtns.append(prevCityEl);
  }
};

//picked a previous city
//submit previous city button
/////add to click target
// var prevCityBtn = document.querySelector(".prevCity");
// var inputs;
// ////fix error on event listener
// prevCityBtn.addEventListener("click",function(event){
//     inputs = {[event.target.id]:event.target.value}
//     console.log(inputs)
//     prevCityName = inputs.value;

// })
// SubmitCityBtn.addEventListener("click", PreviousCityClickHandler);
